...
We use tidyverse and tidymodels.
...
- The workbench is giving problems using data from the web so we will instead download and load them. 
- We then load the data into hfi dataframe and then filter it to create a new dataframe called hfi_2016 which has data justt for the year 2016. 
...
We want to explore the relationship between personal freedom score, `pf_score`, the political pressures and controls on media content index,`pf_expression_control` and Religious freedom `pf_religion`. 

```{r distribution plot}

# Univariate plot for pf_religion (Histogram)
hist(hfi_2016$pf_religion, 
     main = "Distribution of Religious freedom scores (pf_religion)",
     xlab = "Religious Freedom Score",
     col = "skyblue",        # Set the color of the bars
     border = "white",       # Set the color of the bar borders
     xlim = c(0, 10),        # Set x-axis limits for better clarity
     breaks = 20)            # Control the number of bins

# vertical line at the mean for better interpretation
abline(v = mean(hfi_2016$pf_religion, na.rm = TRUE), col = "red", lwd = 4)
...
1. For each pair of variables, how would you describe the relationship graphically?
  ...
  The expression control scores and pf scores seem to be linear. There are outliers in pf_religion data.
...
$$
\hat{y} = 3.4588990 + 0.4692890 \times pf_expression_control + 0.1611585 \times pf_religion
$$
